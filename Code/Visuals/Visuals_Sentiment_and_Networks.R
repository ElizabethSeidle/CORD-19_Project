...
#### Network Visuals based on cluster keywords ####
#NOTELfirst need to run "NetworkPrep_CreateKeywordEdges.ipynb" to generate edges for graphic

library(igraph)
library(networkD3)
library(htmlwidgets)

edges_lg <- read.csv("largeEdgresR.csv", header=T, fileEncoding="UTF-8-BOM")
edges_sm <- read.csv("smallEdgresR.csv", header=T, fileEncoding="UTF-8-BOM")


simpleNetwork_lg <- simpleNetwork(edges_lg, height="200px", width="200px",
                                  fontSize = 14,               
                                  fontFamily = "serif", 
                                  linkColour = "#666",
                                  zoom = T
...
simpleNetwork_sm <- simpleNetwork(edges_sm, height="1000px", width="1000px",
                                  fontSize = 14,               
                                  fontFamily = "serif", 
                                  linkColour = "#666", 
                                  zoom = T
...
path = "C:/Users/ear51/DAAN- Graduate School/DAAN 888 - Capstone Course/Project/Visuals/Network Visual/cleaned data"
saveWidget(simpleNetwork_lg, file=paste0(path, "/simpleNetwork_lg.html"))
saveWidget(simpleNetwork_sm, file=paste0(path, "/simpleNetwork_sm.html"))
